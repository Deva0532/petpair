...
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign(
      { userId: user._id, email: user.email, name: user.name, location: user.location, phone: user.phone, bio: user.bio },
      JWT_SECRET,
      { expiresIn: '1h' }
    );
...
app.post('/api/register', async (req, res) => {
  ...
  try {
    ...
    await newUser.save();
    const token = jwt.sign(
      { userId: newUser._id, email: newUser.email, name: newUser.name, location: newUser.location, phone: newUser.phone, bio: newUser.bio },
      JWT_SECRET,
      { expiresIn: '1h' }
    );
...
app.put('/api/profile', protect, async (req, res) => {
  ...
  try {
    ...
    const newToken = jwt.sign(
      { userId: updatedUser._id, email: updatedUser.email, name: updatedUser.name, location: updatedUser.location, phone: updatedUser.phone, bio: updatedUser.bio },
      JWT_SECRET,
      { expiresIn: '1h' }
    );
...
app.get('/api/pets', async (req, res) => {
  try {
    // Only return active pets for public listing
    const pets = await Pet.find({ status: { $ne: 'deleted' } })
      .sort({ createdAt: -1 })
      .populate('ownerId', 'name email location phone bio');
    res.json(pets);
  } catch (error) {
...
app.put('/api/pets/:id', protect, async (req, res) => {
  try {
    ...
    // Update fields
    const allowedUpdates = ['name', 'breed', 'age', 'type', 'price', 'location', 'description',
      'vaccinated', 'neutered', 'availableForMating', 'availableForSale', 'featured',
      'imageUrls', 'weight', 'personality', 'careRequirements', 'medicalNotes',
      'size', 'activityLevel', 'goodWithKids', 'goodWithPets', 'houseTrained',
      'spayedNeutered', 'specialNeeds', 'status'];
...
app.get('/api/wishlist', protect, async (req, res) => {
  try {
    const wishlistItems = await Wishlist.find({ userId: req.user.userId })
      .populate({
        path: 'petId',
        populate: { path: 'ownerId', select: 'name email location phone bio' }
      })
      .sort({ addedAt: -1 });

    // Map to include pet status for frontend to show availability
