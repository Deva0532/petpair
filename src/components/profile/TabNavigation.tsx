  ...
  return (
    <div className="flex justify-center -mt-10 sm:-mt-12 relative z-20 px-4">
      <div className={`p-1.5 rounded-full backdrop-blur-3xl shadow-xl flex items-center space-x-1 sm:space-x-2 border transition-colors ${
        isAdmin ? 'bg-[#0F172A]/80 border-cyan-500/30' : isKennel ? 'bg-amber-900/5 min-w-max border-amber-200/50 shadow-amber-900/10' : 'bg-white/40 min-w-max border-white/60 shadow-violet-900/5'
      }`}>
        {tabs.map((tab) => {
          const Icon = tab.icon;
            ...
            <button
              ...
              className={`flex items-center space-x-2 py-2.5 px-5 sm:px-6 rounded-full font-bold text-sm sm:text-base transition-all duration-300 ${
                isActive
                  ...
                  : (isAdmin ? 'text-slate-400 hover:text-white hover:bg-white/5' : isKennel ? 'text-amber-900/60 hover:text-amber-900 hover:bg-amber-50/50' : 'text-slate-500 hover:text-slate-800 hover:bg-white/50')
              }`}
            >
